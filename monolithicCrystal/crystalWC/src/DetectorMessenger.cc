...
void DetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  if( command == fMaterialCmd )
    {GlobalPars::Instance()->fMaterial = newValue;}

  if( command == fCrystalWidthCmd )
    {GlobalPars::Instance()->fCrystalWidth = fCrystalWidthCmd->GetNewDoubleValue(newValue);}

  if( command ==  fCrystalX0LengthCmd )
    {GlobalPars::Instance()->fCrystalX0Length = fCrystalX0LengthCmd->GetNewDoubleValue(newValue);}

  if( command ==  fTeflonThicknessCmd )
    {GlobalPars::Instance()->fTeflonThickness = fTeflonThicknessCmd->GetNewDoubleValue(newValue);}
  ...
  if( command ==  fSipmXYCmd )
    {GlobalPars::Instance()->fSipmXY = fSipmXYCmd->GetNewDoubleValue(newValue);}
  if( command ==  fSipmZCmd )
    {GlobalPars::Instance()->fSipmZ = fSipmZCmd->GetNewDoubleValue(newValue);}

  if( command ==  fEpoxyZCmd )
    {GlobalPars::Instance()->fEpoxyZ = fEpoxyZCmd->GetNewDoubleValue(newValue);}
  
  if( command ==  fActiveXYCmd )
    {GlobalPars::Instance()->fActiveXY = fActiveXYCmd->GetNewDoubleValue(newValue);}

  if( command ==  fActiveZCmd )
    {GlobalPars::Instance()->fActiveZ = fActiveZCmd->GetNewDoubleValue(newValue);}
